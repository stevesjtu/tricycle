...
// - Something like "16 bits floats" could be tested, to bypass the int-to-double conversion.
...
#define INIT_QUANTIZATION													\
	...
	CQuantCoeff.x = CMax.x!=0.0 ? double((1<<nbc)-1)/CMax.x : 0.0;			\
	CQuantCoeff.y = CMax.y!=0.0 ? double((1<<nbc)-1)/CMax.y : 0.0;			\
	CQuantCoeff.z = CMax.z!=0.0 ? double((1<<nbc)-1)/CMax.z : 0.0;			\
	EQuantCoeff.x = EMax.x!=0.0 ? double((1<<nbe)-1)/EMax.x : 0.0;			\
	EQuantCoeff.y = EMax.y!=0.0 ? double((1<<nbe)-1)/EMax.y : 0.0;			\
	EQuantCoeff.z = EMax.z!=0.0 ? double((1<<nbe)-1)/EMax.z : 0.0;			\
	/* Compute and save dequantization coeffs */							\
	mCenterCoeff.x = CQuantCoeff.x!=0.0 ? 1.0 / CQuantCoeff.x : 0.0;		\
	mCenterCoeff.y = CQuantCoeff.y!=0.0 ? 1.0 / CQuantCoeff.y : 0.0;		\
	mCenterCoeff.z = CQuantCoeff.z!=0.0 ? 1.0 / CQuantCoeff.z : 0.0;		\
	mExtentsCoeff.x = EQuantCoeff.x!=0.0 ? 1.0 / EQuantCoeff.x : 0.0;	\
	mExtentsCoeff.y = EQuantCoeff.y!=0.0 ? 1.0 / EQuantCoeff.y : 0.0;	\
	mExtentsCoeff.z = EQuantCoeff.z!=0.0 ? 1.0 / EQuantCoeff.z : 0.0;	\
...
#define PERFORM_QUANTIZATION														\
	...
	{																				\
		...
		{	/* Dequantize the box center */											\
			double qc = double(mNodes[i].mAABB.mCenter[j]) * mCenterCoeff[j];			\
			bool FixMe=true;														\
			do																		\
			{	/* Dequantize the box extent */										\
				double qe = double(mNodes[i].mAABB.mExtents[j]) * mExtentsCoeff[j];	\
