...
#include "opcode/OPC_TriTriOverlap.h"
...
void AABBTreeCollider::InitQuery(const Matrix4x4* world0, const Matrix4x4* world1)
{
	...
	for(udword i=0;i<3;i++)
	{
		for(udword j=0;j<3;j++)
		{
			...
			mAR.m[i][j] = 1e-15 + abs(mR1to0.m[i][j]);
		}
	}
}
...
bool AABBTreeCollider::Collide(const AABBQuantizedTree* tree0, const AABBQuantizedTree* tree1, const Matrix4x4* world0, const Matrix4x4* world1, Pair* cache)
{
	...
	const Point a(double(N0->mAABB.mExtents[0]) * mExtentsCoeff0.x, double(N0->mAABB.mExtents[1]) * mExtentsCoeff0.y, double(N0->mAABB.mExtents[2]) * mExtentsCoeff0.z);
	const Point Pa(double(N0->mAABB.mCenter[0]) * mCenterCoeff0.x, double(N0->mAABB.mCenter[1]) * mCenterCoeff0.y, double(N0->mAABB.mCenter[2]) * mCenterCoeff0.z);
	// Dequantize box B
	const AABBQuantizedNode* N1 = tree1->GetNodes();
	const Point b(double(N1->mAABB.mExtents[0]) * mExtentsCoeff1.x, double(N1->mAABB.mExtents[1]) * mExtentsCoeff1.y, double(N1->mAABB.mExtents[2]) * mExtentsCoeff1.z);
	const Point Pb(double(N1->mAABB.mCenter[0]) * mCenterCoeff1.x, double(N1->mAABB.mCenter[1]) * mCenterCoeff1.y, double(N1->mAABB.mCenter[2]) * mCenterCoeff1.z);
...
void AABBTreeCollider::PrimTest(udword id0, udword id1)
{
	...
	IceCore::contactStruct contarg;
	if (TriTriOverlap(*VP0.Vertex[0], *VP0.Vertex[1], *VP0.Vertex[2], u0, u1, u2, contarg))
	{
		// Keep track of colliding pairs
		mPairs.Add(id0).Add(id1);
		mPairs.ContactArgAdd(contarg);
		// Set contact status
		mFlags |= OPC_CONTACT;
	}
}
...
inline_ void AABBTreeCollider::PrimTestTriIndex(udword id1)
{
	...
	// Perform triangle-triangle overlap test
	IceCore::contactStruct contarg;
	if(TriTriOverlap(mLeafVerts[0], mLeafVerts[1], mLeafVerts[2], *VP.Vertex[0], *VP.Vertex[1], *VP.Vertex[2], contarg))
	{
		// Keep track of colliding pairs
...
inline_ void AABBTreeCollider::PrimTestIndexTri(udword id0)
{
	...
	// Perform triangle-triangle overlap test
	IceCore::contactStruct contarg;
	if(TriTriOverlap( *VP.Vertex[0], *VP.Vertex[1], *VP.Vertex[2],mLeafVerts[0], mLeafVerts[1], mLeafVerts[2], contarg))
	{
		// Keep track of colliding pairs
...
void AABBTreeCollider::_Collide(const AABBQuantizedNode* b0, const AABBQuantizedNode* b1, const Point& a, const Point& Pa, const Point& b, const Point& Pb)
{
	...
	if(b1->IsLeaf() || (!b0->IsLeaf() && (b0->GetSize() > b1->GetSize())))
	{
		...
		const Point negPa(double(Box->mCenter[0]) * mCenterCoeff0.x, double(Box->mCenter[1]) * mCenterCoeff0.y, double(Box->mCenter[2]) * mCenterCoeff0.z);
		const Point nega(double(Box->mExtents[0]) * mExtentsCoeff0.x, double(Box->mExtents[1]) * mExtentsCoeff0.y, double(Box->mExtents[2]) * mExtentsCoeff0.z);
		...
		// Dequantize box
		Box = &b0->GetPos()->mAABB;
		const Point posPa(double(Box->mCenter[0]) * mCenterCoeff0.x, double(Box->mCenter[1]) * mCenterCoeff0.y, double(Box->mCenter[2]) * mCenterCoeff0.z);
		const Point posa(double(Box->mExtents[0]) * mExtentsCoeff0.x, double(Box->mExtents[1]) * mExtentsCoeff0.y, double(Box->mExtents[2]) * mExtentsCoeff0.z);
	...
	else
	{
		...
		const Point negPb(double(Box->mCenter[0]) * mCenterCoeff1.x, double(Box->mCenter[1]) * mCenterCoeff1.y, double(Box->mCenter[2]) * mCenterCoeff1.z);
		const Point negb(double(Box->mExtents[0]) * mExtentsCoeff1.x, double(Box->mExtents[1]) * mExtentsCoeff1.y, double(Box->mExtents[2]) * mExtentsCoeff1.z);
		...
		// Dequantize box
		Box = &b1->GetPos()->mAABB;
		const Point posPb(double(Box->mCenter[0]) * mCenterCoeff1.x, double(Box->mCenter[1]) * mCenterCoeff1.y, double(Box->mCenter[2]) * mCenterCoeff1.z);
		const Point posb(double(Box->mExtents[0]) * mExtentsCoeff1.x, double(Box->mExtents[1]) * mExtentsCoeff1.y, double(Box->mExtents[2]) * mExtentsCoeff1.z);
...
void AABBTreeCollider::_CollideTriBox(const AABBQuantizedNoLeafNode* b)
{
	...
	const Point Pb(double(bb->mCenter[0]) * mCenterCoeff1.x, double(bb->mCenter[1]) * mCenterCoeff1.y, double(bb->mCenter[2]) * mCenterCoeff1.z);
	const Point eb(double(bb->mExtents[0]) * mExtentsCoeff1.x, double(bb->mExtents[1]) * mExtentsCoeff1.y, double(bb->mExtents[2]) * mExtentsCoeff1.z);
...
void AABBTreeCollider::_CollideBoxTri(const AABBQuantizedNoLeafNode* b)
{
	// Dequantize box
	const QuantizedAABB* bb = &b->mAABB;
	const Point Pa(double(bb->mCenter[0]) * mCenterCoeff0.x, double(bb->mCenter[1]) * mCenterCoeff0.y, double(bb->mCenter[2]) * mCenterCoeff0.z);
	const Point ea(double(bb->mExtents[0]) * mExtentsCoeff0.x, double(bb->mExtents[1]) * mExtentsCoeff0.y, double(bb->mExtents[2]) * mExtentsCoeff0.z);
...
void AABBTreeCollider::_Collide(const AABBQuantizedNoLeafNode* a, const AABBQuantizedNoLeafNode* b)
{
	...
	const Point Pa(double(ab->mCenter[0]) * mCenterCoeff0.x, double(ab->mCenter[1]) * mCenterCoeff0.y, double(ab->mCenter[2]) * mCenterCoeff0.z);
	const Point ea(double(ab->mExtents[0]) * mExtentsCoeff0.x, double(ab->mExtents[1]) * mExtentsCoeff0.y, double(ab->mExtents[2]) * mExtentsCoeff0.z);
	// Dequantize box B
	const QuantizedAABB* bb = &b->mAABB;
	const Point Pb(double(bb->mCenter[0]) * mCenterCoeff1.x, double(bb->mCenter[1]) * mCenterCoeff1.y, double(bb->mCenter[2]) * mCenterCoeff1.z);
	const Point eb(double(bb->mExtents[0]) * mExtentsCoeff1.x, double(bb->mExtents[1]) * mExtentsCoeff1.y, double(bb->mExtents[2]) * mExtentsCoeff1.z);
