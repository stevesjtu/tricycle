...
BOOL OBBCollider::InitQuery(OBBCache& cache, const OBB& box, const Matrix4x4* worldb, const Matrix4x4* worldm)
{
	...
	for(udword i=0;i<3;i++)
	{
		for(udword j=0;j<3;j++)
		{
			...
			mAR.m[i][j] = 1e-6 + fabs(mRBoxToModel.m[i][j]);
		}
	}
...
*/
	...
	// - test AABB-in-AABB
	double NCx = bc.x * mRModelToBox.m[0][0] + bc.y * mRModelToBox.m[1][0] + bc.z * mRModelToBox.m[2][0];
	double NEx = fabs(mRModelToBox.m[0][0] * be.x) + fabs(mRModelToBox.m[1][0] * be.y) + fabs(mRModelToBox.m[2][0] * be.z);

	if(mB0.x < NCx+NEx)	return FALSE;
	if(mB1.x > NCx-NEx)	return FALSE;

	double NCy = bc.x * mRModelToBox.m[0][1] + bc.y * mRModelToBox.m[1][1] + bc.z * mRModelToBox.m[2][1];
	double NEy = fabs(mRModelToBox.m[0][1] * be.x) + fabs(mRModelToBox.m[1][1] * be.y) + fabs(mRModelToBox.m[2][1] * be.z);

	if(mB0.y < NCy+NEy)	return FALSE;
	if(mB1.y > NCy-NEy)	return FALSE;

	double NCz = bc.x * mRModelToBox.m[0][2] + bc.y * mRModelToBox.m[1][2] + bc.z * mRModelToBox.m[2][2];
	double NEz = fabs(mRModelToBox.m[0][2] * be.x) + fabs(mRModelToBox.m[1][2] * be.y) + fabs(mRModelToBox.m[2][2] * be.z);
...
void OBBCollider::_Collide(const AABBQuantizedNode* node)
{
	...
	const Point Center(double(Box.mCenter[0]) * mCenterCoeff.x, double(Box.mCenter[1]) * mCenterCoeff.y, double(Box.mCenter[2]) * mCenterCoeff.z);
	const Point Extents(double(Box.mExtents[0]) * mExtentsCoeff.x, double(Box.mExtents[1]) * mExtentsCoeff.y, double(Box.mExtents[2]) * mExtentsCoeff.z);
