...
inline_ BOOL AABBTreeCollider::BoxBoxOverlap(const Point& ea, const Point& ca, const Point& eb, const Point& cb)
{
	...
	double Tx = (mR1to0.m[0][0]*cb.x + mR1to0.m[1][0]*cb.y + mR1to0.m[2][0]*cb.z) + mT1to0.x - ca.x;
	t = ea.x + eb.x*mAR.m[0][0] + eb.y*mAR.m[1][0] + eb.z*mAR.m[2][0];
	if(GREATER(Tx, t))	return FALSE;

	double Ty = (mR1to0.m[0][1]*cb.x + mR1to0.m[1][1]*cb.y + mR1to0.m[2][1]*cb.z) + mT1to0.y - ca.y;
	t = ea.y + eb.x*mAR.m[0][1] + eb.y*mAR.m[1][1] + eb.z*mAR.m[2][1];
	if(GREATER(Ty, t))	return FALSE;

	double Tz = (mR1to0.m[0][2]*cb.x + mR1to0.m[1][2]*cb.y + mR1to0.m[2][2]*cb.z) + mT1to0.z - ca.z;
...
inline_ BOOL OBBCollider::BoxBoxOverlap(const Point& extents, const Point& center)
{
	...
	// Class I : A's basis vectors
	double Tx = mTBoxToModel.x - center.x;	t = extents.x + mBBx1;	if(GREATER(Tx, t))	return FALSE;
	double Ty = mTBoxToModel.y - center.y;	t = extents.y + mBBy1;	if(GREATER(Ty, t))	return FALSE;
	double Tz = mTBoxToModel.z - center.z;	t = extents.z + mBBz1;	if(GREATER(Tz, t))	return FALSE;
...
inline_ BOOL AABBCollider::AABBAABBOverlap(const Point& extents, const Point& center)
{
	...
	double tx = mBox.mCenter.x - center.x;	double ex = extents.x + mBox.mExtents.x;	if(GREATER(tx, ex))	return FALSE;
	double ty = mBox.mCenter.y - center.y;	double ey = extents.y + mBox.mExtents.y;	if(GREATER(ty, ey))	return FALSE;
	double tz = mBox.mCenter.z - center.z;	double ez = extents.z + mBox.mExtents.z;	if(GREATER(tz, ez))	return FALSE;
