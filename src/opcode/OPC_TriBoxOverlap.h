...
//! TO BE DOCUMENTED
inline_ BOOL planeBoxOverlap(const Point& normal, const double d, const Point& maxbox)
{
	Point vmin, vmax;
	for(udword q=0;q<=2;q++)
	{
		if(normal[q]>0.0)	{ vmin[q]=-maxbox[q]; vmax[q]=maxbox[q]; }
