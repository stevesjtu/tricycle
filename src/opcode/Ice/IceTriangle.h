...
#define __ICETRIANGLE_H__
	...
	class ICEMATHS_API Triangle
	{
		...
		inline_	Plane			PlaneEquation() const	{ return Plane(mVerts[0], mVerts[1], mVerts[2]);	}
				...
				PartVal			TestAgainstPlane(const Plane& plane, double epsilon) const;
//				double			Distance(Point& cp, Point& cq, Tri& tri);
				void			ComputeMoment(Moment& m);
				double			MinEdgeLength() const;
				double			MaxEdgeLength() const;
				void			ComputePoint(double u, double v, Point& pt, udword* nearvtx=null)	const;
				void			Inflate(double fat_coeff, bool constant_border);
	};
