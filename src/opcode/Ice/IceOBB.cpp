...
bool OBB::ContainsPoint(const Point& p) const
{
	...
	double f = mRot.m[0][0] * RelPoint.x + mRot.m[0][1] * RelPoint.y + mRot.m[0][2] * RelPoint.z;
...
BOOL OBB::IsInside(const OBB& box) const
{
	...
	double f = abs(mtx.m[0][0] * mExtents.x) + abs(mtx.m[1][0] * mExtents.y) + abs(mtx.m[2][0] * mExtents.z) - box.mExtents.x;
	if(f > _1in0.mCenter.x)		return FALSE;
	if(-f < _1in0.mCenter.x)	return FALSE;

	f = abs(mtx.m[0][1] * mExtents.x) + abs(mtx.m[1][1] * mExtents.y) + abs(mtx.m[2][1] * mExtents.z) - box.mExtents.y;
	if(f > _1in0.mCenter.y)		return FALSE;
	if(-f < _1in0.mCenter.y)	return FALSE;

	f = abs(mtx.m[0][2] * mExtents.x) + abs(mtx.m[1][2] * mExtents.y) + abs(mtx.m[2][2] * mExtents.z) - box.mExtents.z;
