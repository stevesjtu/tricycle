...
#define AABB_COMMON_METHODS																											\
			...
			double			MakeCube(AABB& cube)																			const;	\
			void			MakeSphere(Sphere& sphere)																		const;	\
			const sbyte*	ComputeOutline(const Point& local_eye, sdword& num)												const;	\
			double			ComputeBoxArea(const Point& eye, const Matrix4x4& mat, double width, double height, sdword& num)	const;	\
	...
	class ICEMATHS_API AABB
	{
		 ...
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						double		GetSize()								const		{ Point e; GetExtents(e);	return e.Max();	}
		...
		inline_			double		GetMin(udword axis)						const		{ return mMin[axis];						}
		//! Get component of the box's max point along a given axis
		inline_			double		GetMax(udword axis)						const		{ return mMax[axis];						}

		//! Get box center
		inline_			void		GetCenter(Point& center)				const		{ center = (mMax + mMin)*0.5;				}
		//! Get box extents
		inline_			void		GetExtents(Point& extents)				const		{ extents = (mMax - mMin)*0.5;				}

		//! Get component of the box's center along a given axis
		inline_			double		GetCenter(udword axis)					const		{ return (mMax[axis] + mMin[axis])*0.5;	}
		//! Get component of the box's extents along a given axis
		inline_			double		GetExtents(udword axis)					const		{ return (mMax[axis] - mMin[axis])*0.5;	}

		//! Get box diagonal
		inline_			void		GetDiagonal(Point& diagonal)			const		{ diagonal = mMax - mMin;					}
		inline_			double		GetWidth()								const		{ return mMax.x - mMin.x;					}
		inline_			double		GetHeight()								const		{ return mMax.y - mMin.y;					}
		inline_			double		GetDepth()								const		{ return mMax.z - mMin.z;					}

		//! Volume
		inline_			double		GetVolume()								const		{ return GetWidth() * GetHeight() * GetDepth();		}
		...
		//! Operator for AABB *= double. Scales the extents, keeps same center.
		inline_			AABB&		operator*=(double s)
									{
	...
	class ICEMATHS_API AABB
	{
		 ...
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void		SetMinMax(const Point& min, const Point& max)		{ mCenter = (max + min)*0.5; mExtents = (max - min)*0.5;		}
		 ...
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						double		GetSize()								const		{ return mExtents.Max();					}
		...
		//! Get component of the box's min point along a given axis
		inline_			double		GetMin(udword axis)						const		{ return mCenter[axis] - mExtents[axis];	}
		//! Get component of the box's max point along a given axis
		inline_			double		GetMax(udword axis)						const		{ return mCenter[axis] + mExtents[axis];	}
		...
		//! Get component of the box's center along a given axis
		inline_			double		GetCenter(udword axis)					const		{ return mCenter[axis];						}
		//! Get component of the box's extents along a given axis
		inline_			double		GetExtents(udword axis)					const		{ return mExtents[axis];					}

		//! Get box diagonal
		inline_			void		GetDiagonal(Point& diagonal)			const		{ diagonal = mExtents * 2.0;				}
		inline_			double		GetWidth()								const		{ return mExtents.x * 2.0;					}
		inline_			double		GetHeight()								const		{ return mExtents.y * 2.0;					}
		inline_			double		GetDepth()								const		{ return mExtents.z * 2.0;					}

		//! Volume
		inline_			double		GetVolume()								const		{ return mExtents.x * mExtents.y * mExtents.z * 8.0;	}
		...
		inline_			BOOL		Intersect(const AABB& a)				const
									{
										double tx = mCenter.x - a.mCenter.x;	double ex = a.mExtents.x + mExtents.x;	if(AIR(tx) > IR(ex))	return FALSE;
										double ty = mCenter.y - a.mCenter.y;	double ey = a.mExtents.y + mExtents.y;	if(AIR(ty) > IR(ey))	return FALSE;
										double tz = mCenter.z - a.mCenter.z;	double ez = a.mExtents.z + mExtents.z;	if(AIR(tz) > IR(ez))	return FALSE;
										return TRUE;
									}
		...
		inline_			bool		GomezIntersect(const AABB& a)
									{
										...
										return	((abs(T.x) <= (a.mExtents.x + mExtents.x))
												&& (abs(T.y) <= (a.mExtents.y + mExtents.y))
												&& (abs(T.z) <= (a.mExtents.z + mExtents.z)));
									}
		...
		inline_			BOOL		Intersect(const AABB& a, udword axis)	const
									{
										double t = mCenter[axis] - a.mCenter[axis];
										double e = a.mExtents[axis] + mExtents[axis];
		...
		//! Operator for AABB *= double. Scales the extents, keeps same center.
		inline_			AABB&		operator*=(double s)		{ mExtents*=s;	return *this;	}
