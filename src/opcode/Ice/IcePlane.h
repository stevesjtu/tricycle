...
#define __ICEPLANE_H__
	...
	class ICEMATHS_API Plane
	{
		...
		inline_			Plane(double nx, double ny, double nz, double d)					{ Set(nx, ny, nz, d);							}
		...
		//! Constructor from a normal and a distance
		inline_			Plane(const Point& _n, double _d)								{ n = _n; d = _d;								}
		//! Copy constructor
		inline_			Plane(const Plane& plane) : n(plane.n), d(plane.d)				{												}
		//! Destructor
		inline_			~Plane()														{												}

		inline_	Plane&	Zero()															{ n.Zero(); d = 0.0;			return *this;	}
		inline_	Plane&	Set(double nx, double ny, double nz, double _d)						{ n.Set(nx, ny, nz); d = _d;	return *this;	}
		...
		inline_	double	Distance(const Point& p)			const						{ return (p | n) + d;							}
		inline_	bool	Belongs(const Point& p)				const						{ return abs(Distance(p)) < PLANE_EPSILON;	}

		inline_	void	Normalize()
						{
							double Denom = 1.0 / n.Magnitude();
							n.x	*= Denom;
							n.y	*= Denom;
							n.z	*= Denom;
							d	*= Denom;
		...
		// Members
				...
				double	d;		//!< The distance from the origin
		...
		inline_	Plane&	operator*=(const Matrix4x4& m)
						{
							// Old code from Irion. Kept for reference.
							Point n2 = HPoint(n, 0.0) * m;
							d = -((Point) (HPoint( -d*n, 1.0 ) * m) | n2);
							n = n2;
							return *this;
						}
	};
