...
{
	x = UnitRandomFloat() - 0.5;
	y = UnitRandomFloat() - 0.5;
	z = UnitRandomFloat() - 0.5;
	Normalize();
	return *this;
}
...
// Cast operator
// WARNING: not inlined
Point::operator HPoint() const	{ return HPoint(x, y, z, 0.0); }

Point& Point::Refract(const Point& eye, const Point& n, double refractindex, Point& refracted)
{
...
void Point::ProjectToScreen(double halfrenderwidth, double halfrenderheight, const Matrix4x4& mat, HPoint& projected) const
{
	projected = HPoint(x, y, z, 1.0) * mat;
	projected.w = 1.0 / projected.w;
...
void Point::SetNotUsed()
{
	...
	x = y = z = FR(0xffffffffffffffff);
}

BOOL Point::IsNotUsed()	const
{
	if(IR(x)!=0xffffffffffffffff)	return FALSE;
	if(IR(y)!=0xffffffffffffffff)	return FALSE;
	if(IR(z)!=0xffffffffffffffff)	return FALSE;
	return TRUE;
}
...
{
	double sx = r.x - linpos.x;
	double sy = r.y - linpos.y;
	double sz = r.z - linpos.z;
