...
#define __ICEMATRIX3X3_H__
	...
	#define	MATRIX3X3_EPSILON		(1.0e-15)
	...
	class ICEMATHS_API Matrix3x3
	{
		...
		inline_					Matrix3x3(double m00, double m01, double m02, double m10, double m11, double m12, double m20, double m21, double m22)
								{
		...
		//! Copy constructor
		inline_					Matrix3x3(const Matrix3x3& mat)				{ CopyMemory(m, &mat.m, 9*sizeof(double));	}
		//! Destructor
		inline_					~Matrix3x3()								{}

		//! Assign values
		inline_	void			Set(double m00, double m01, double m02, double m10, double m11, double m12, double m20, double m21, double m22)
								{
									m[0][0] = m00;	m[0][1] = m01;	m[0][2] = m02;
									m[1][0] = m10;	m[1][1] = m11;	m[1][2] = m12;
									m[2][0] = m20;	m[2][1] = m21;	m[2][2] = m22;
		...
		inline_	void			Scale(double sx, double sy, double sz)
								{
		...
		inline_	void			Copy(const Matrix3x3& source)				{ CopyMemory(m, source.m, 9*sizeof(double));			}
		...
		inline_	double			Trace()					const				{ return m[0][0] + m[1][1] + m[2][2];				}
		//! Clears the matrix.
		inline_	void			Zero()										{ ZeroMemory(&m, sizeof(m));						}
		//! Sets the identity matrix.
		inline_	void			Identity()									{ Zero(); m[0][0] = m[1][1] = m[2][2] = 1.0; 		}
		//! Checks for identity
		...
		//! Mac
		inline_	void			Mac(const Matrix3x3& a, const Matrix3x3& b, double s)
								{
		...
		//! Mac
		inline_	void			Mac(const Matrix3x3& a, double s)
								{
		...
		//! this = A * s
		inline_	void			Mult(const Matrix3x3& a, double s)
								{
									m[0][0] = a.m[0][0] * s;	m[0][1] = a.m[0][1] * s;	m[0][2] = a.m[0][2] * s;
									m[1][0] = a.m[1][0] * s;	m[1][1] = a.m[1][1] * s;	m[1][2] = a.m[1][2] * s;
									m[2][0] = a.m[2][0] * s;	m[2][1] = a.m[2][1] * s;	m[2][2] = a.m[2][2] * s;
		...
		//!	RY.RX	0		cx			sx
		//!			sy		-sx.cy		cx.cy
				void			RotYX(double y, double x);

		//! Make a rotation matrix about an arbitrary axis
				Matrix3x3&		Rot(double angle, const Point& axis);
...
/*
		//! Compute a cofactor. Used for matrix inversion.
				double			CoFactor(ubyte row, ubyte column)	const
				{
...
*/
		//! Invert the matrix. Determinant must be different from zero, else matrix can't be inverted.
				Matrix3x3&		Invert()
								{
									double Det = Determinant();	// Must be !=0
...
void FromQuatL2(const Quat &q, double l2);
		...
		//! Operator for Matrix3x3 Mul = Matrix3x3 * double;
		inline_	Matrix3x3		operator*(double s)				const
								{
									return Matrix3x3(
		...
		//! Operator for Matrix3x3 Mul = double * Matrix3x3;
		inline_	friend Matrix3x3 operator*(double s, const Matrix3x3& mat)
								{
									return Matrix3x3(
		...
		//! Operator for Matrix3x3 Div = Matrix3x3 / double;
		inline_	Matrix3x3		operator/(double s)				const
								{
									if (s)	s = 1.0 / s;
									return Matrix3x3(
		...
		//! Operator for Matrix3x3 Div = double / Matrix3x3;
		inline_	friend Matrix3x3 operator/(double s, const Matrix3x3& mat)
								{
									return Matrix3x3(
									s/mat.m[0][0],	s/mat.m[0][1],	s/mat.m[0][2],
		...
		//! Operator for Matrix3x3 *= double
		inline_	Matrix3x3&		operator*=(double s)
								{
									m[0][0] *= s;	m[0][1] *= s;	m[0][2] *= s;
									m[1][0] *= s;	m[1][1] *= s;	m[1][2] *= s;
									m[2][0] *= s;	m[2][1] *= s;	m[2][2] *= s;
