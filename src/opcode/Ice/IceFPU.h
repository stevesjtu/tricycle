...
#define __ICEFPU_H__
	...
	#define	SIGN_BITMASK			0x8000000000000000
	#include <cmath> // in Linux with intel compiler

	namespace {
		union double_udword { double f; unsigned __int64 u; };
		union double_sdword { double f; __int64 s; };

		union float_udword { float f; unsigned u; };
	}


	//! Integer representation of a floating-point value.
	//#define IR(x)					((udword&)(x))
	static inline unsigned __int64 IR(double x) { double_udword fu; fu.f = x; return fu.u; }
	static inline unsigned IR(float x) { float_udword fu; fu.f = x; return fu.u; }
	//! Signed integer representation of a floating-point value.
	//#define SIR(x)					((sdword&)(x))
	static inline __int64 SIR(double x) { double_sdword fs; fs.f = x; return fs.s; }

	//! Absolute integer representation of a floating-point value
	#define AIR(x)					(IR(x)&0x7fffffffffffffff)

	//! Floating-point representation of an integer value.
	//#define FR(x)					((double&)(x))
	static inline double FR(unsigned __int64 x) { double_udword fu; fu.u = x; return fu.f; }
	...
	//inline_ double FastFabs(double x)
	//{
	...
	//inline_ double FastSqrt(double square)
	//{
	//	return sqrt(square);
	//}

	////! Saturates positive to zero.
	//inline_ double fsat(double f)
	//{
	...
	//inline_ double frsqrt(double f)
	//{
	//	double x = f * 0.5f;
	//	udword y = 0x5f3759df - (IR(f) >> 1);
	//	// Iteration...
	//	const double fy = FR(y);
	//	const double result = fy * ( 1.5f - ( x * fy * fy ) );
	//	// Result
	//	return result;
	//}

	////! Computes 1.0f / sqrtf(x). Comes from NVIDIA.
	//inline_ double InvSqrt(const double& x)
	//{
	//	const udword tmp = (udword(IEEE_1_0 << 1) + IEEE_1_0 - IR(x)) >> 1;
	//	const double y = FR(tmp);
	...
	//inline_ double RSqrt(double number)
	//{
	//	int i;
	//	double x2, y;
	...
	////! TO BE DOCUMENTED
	//inline_ double fsqrt(double f)
	//{
	//	udword y = ( ( SIR(f) - 0x3f800000 ) >> 1 ) + 0x3f800000;
	//	// Iteration...?
	//	// (double&)y = (3.0f - ((double&)y * (double&)y) / f) * (double&)y * 0.5f;
	//	// Result
	//	return FR(y);
	//}

	////! Returns the double ranged espilon value.
	//inline_ double fepsilon(double f)
	//{
	...
	//! Is the double valid ?
	//inline_ bool IsNAN(double value)				{ return (IR(value)&0x7f800000) == 0x7f800000;	}
	//inline_ bool IsIndeterminate(double value)	{ return IR(value) == 0xffc00000;				}
	//inline_ bool IsPlusInf(double value)			{ return IR(value) == 0x7f800000;				}
	//inline_ bool IsMinusInf(double value)		{ return IR(value) == 0xff800000;				}

	inline_	bool IsValidFloat(double value)
	{
		//if(IsNAN(value))			return false;
		//if(IsIndeterminate(value))	return false;
		//if(IsPlusInf(value))		return false;
		//if(IsMinusInf(value))		return false;
		return !(std::isnan(value)|| std::isinf(value));  //#include <cmath> in Linux with intel compiler
	}
...
*/
	...
	//inline_ double ComputeFloatEpsilon()
	//{
	//	const double f = FR( IR(1.0f) ^ 1 );
	//	return f - 1.0f;	// You can check it's the same as FLT_EPSILON
	//}

	//inline_ bool IsFloatZero(double x, double epsilon=1e-6f)
	//{
	//	return x*x < epsilon;
	//}
	...
	inline_ double FCMax2(double a, double b)
	{
		return (a > b) ? a : b;
	}

	//! A global function to find MIN(a,b) using FCOMI/FCMOV
	inline_ double FCMin2(double a, double b)
	{
		return (a < b) ? a : b;
	}

	//! A global function to find MAX(a,b,c) using FCOMI/FCMOV
	inline_ double FCMax3(double a, double b, double c)
	{
		return (a > b) ? ((a > c) ? a : c) : ((b > c) ? b : c);
	}

	//! A global function to find MIN(a,b,c) using FCOMI/FCMOV
	inline_ double FCMin3(double a, double b, double c)
	{
		return (a < b) ? ((a < c) ? a : c) : ((b < c) ? b : c);
	}

	//inline_ int ConvertToSortable(double f)
	//{
	//	int Fi = SIR(f);
	//	int Fmask = (Fi>>31);
	//	Fi ^= Fmask;
	...
	//FUNCTION ICECORE_API int		intChop(const double& f);
	//FUNCTION ICECORE_API int		intFloor(const double& f);
	//FUNCTION ICECORE_API int		intCeil(const double& f);
