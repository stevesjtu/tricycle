...
#define __ICEMATRIX4X4_H__
	...
	#define	MATRIX4X4_EPSILON		(1.0e-15)

	class ICEMATHS_API Matrix4x4
	{
//				void	LUBackwardSubstitution( sdword *indx, double* b );
//				void	LUDecomposition( sdword* indx, double* d );

		public:
		//! Empty constructor.
		inline_						Matrix4x4()									{}
		//! Constructor from 16 values
		inline_						Matrix4x4(	double m00, double m01, double m02, double m03,
		...
		//! Copy constructor
		inline_						Matrix4x4(const Matrix4x4& mat)				{ CopyMemory(m, &mat.m, 16*sizeof(double));	}
		//! Destructor.
		inline_						~Matrix4x4()								{}

		//! Assign values (rotation only)
		inline_	Matrix4x4&			Set(	double m00, double m01, double m02,
		...
		//! Assign values
		inline_	Matrix4x4&			Set(	double m00, double m01, double m02, double m03,
											double m10, double m11, double m12, double m13,
											double m20, double m21, double m22, double m23,
											double m30, double m31, double m32, double m33)
									{
		...
		inline_	void				Copy(const Matrix4x4& source)				{ CopyMemory(m, source.m, 16*sizeof(double));	}
		...
		//! Sets a row.
		inline_	void				SetRow(const udword r, const Point& p)		{ m[r][0]=p.x;	m[r][1]=p.y;	m[r][2]=p.z;	m[r][3]= (r!=3) ? 0.0 : 1.0;		}
		//! Returns a column.
		...
		//! Sets a column.
		inline_	void				SetCol(const udword c, const Point& p)		{ m[0][c]=p.x;	m[1][c]=p.y;	m[2][c]=p.z;	m[3][c]= (c!=3) ? 0.0 : 1.0;	}
		...
		inline_	void				SetTrans(double tx, double ty, double tz)		{ m[3][0]=tx;	m[3][1]=ty;		m[3][2]=tz;		}

		// Scale
		//! Sets the scale from a Point. The point is put on the diagonal.
		inline_	void				SetScale(const Point& p)					{ m[0][0]=p.x;	m[1][1]=p.y;	m[2][2]=p.z;	}
		//! Sets the scale from floats. Values are put on the diagonal.
		inline_	void				SetScale(double sx, double sy, double sz)		{ m[0][0]=sx;	m[1][1]=sy;		m[2][2]=sz;		}
...
/*
		...
		//! Sets a column.
						Matrix4x4&	SetCol(const udword col, const Point& p)
						{
							...
							m[1][col] = p.y;
							m[2][col] = p.z;
							m[3][col] = (col != 3) ? 0.0 : 1.0;
							return	*this;
						}
*/
		//! Computes the trace. The trace is the sum of the 4 diagonal components.
		inline_	double				Trace()			const			{ return m[0][0] + m[1][1] + m[2][2] + m[3][3];			}
		//! Computes the trace of the upper 3x3 matrix.
		inline_	double				Trace3x3()		const			{ return m[0][0] + m[1][1] + m[2][2];					}
		//! Clears the matrix.
		inline_	void				Zero()							{ ZeroMemory(&m,  sizeof(m));							}
		//! Sets the identity matrix.
		inline_	void				Identity()						{ Zero(); m[0][0] = m[1][1] = m[2][2] = m[3][3] = 1.0;	}
		//! Checks for identity
		...
		//! Sets a rotation matrix around the X axis.
				void				RotX(double angle)	{ double Cos = cos(angle), Sin = sin(angle); Identity(); m[1][1] = m[2][2] = Cos; m[2][1] = -Sin;	m[1][2] = Sin;	}
		//! Sets a rotation matrix around the Y axis.
				void				RotY(double angle)	{ double Cos = cos(angle), Sin = sin(angle); Identity(); m[0][0] = m[2][2] = Cos; m[2][0] = Sin;	m[0][2] = -Sin;	}
		//! Sets a rotation matrix around the Z axis.
				void				RotZ(double angle)	{ double Cos = cos(angle), Sin = sin(angle); Identity(); m[0][0] = m[1][1] = Cos; m[1][0] = -Sin;	m[0][1] = Sin;	}

		//! Makes a rotation matrix about an arbitrary axis
				Matrix4x4&			Rot(double angle, Point& p1, Point& p2);
...
//				Matrix&	ComputeAxisMatrix(Point& axis, double angle);
		...
		//! Operator for Matrix4x4 Scale = Matrix4x4 * double;
		inline_	Matrix4x4			operator*(double s)				const
									{
										return Matrix4x4(
		...
		//! Operator for Matrix4x4 Scale = double * Matrix4x4;
		inline_	friend Matrix4x4	operator*(double s, const Matrix4x4& mat)
									{
										return Matrix4x4(
		...
		//! Operator for Matrix4x4 Div = Matrix4x4 / double;
		inline_	Matrix4x4			operator/(double s)				const
									{
										if(s) s = 1.0 / s;
		...
		//! Operator for Matrix4x4 Div = double / Matrix4x4;
		inline_	friend Matrix4x4	operator/(double s, const Matrix4x4& mat)
									{
										return Matrix4x4(
										s/mat.m[0][0],	s/mat.m[0][1],	s/mat.m[0][2],	s/mat.m[0][3],
		...
		//! Operator for Matrix4x4 *= double;
		inline_	Matrix4x4&		operator*=(double s)
								{
									m[0][0]*=s;	m[0][1]*=s;	m[0][2]*=s;	m[0][3]*=s;
									m[1][0]*=s;	m[1][1]*=s;	m[1][2]*=s;	m[1][3]*=s;
									m[2][0]*=s;	m[2][1]*=s;	m[2][2]*=s;	m[2][3]*=s;
