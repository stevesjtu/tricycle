...
#define __ICEINDEXEDTRIANGLE_H__
	...
	class ICEMATHS_API IndexedTriangle
	{
		...
		// Methods
				void			Flip();
				double			Area(const Point* verts)											const;
				double			Perimeter(const Point* verts)										const;
				double			Compacity(const Point* verts)										const;
				...
				double			ComputeOcclusionPotential(const Point* verts, const Point& view)	const;
		...
		inline_	udword		OppositeVertex(ubyte edgenb)										const	{ return mVRef[2-edgenb];	}
				...
				double			MinEdgeLength(const Point* verts)									const;
				double			MaxEdgeLength(const Point* verts)									const;
				void			ComputePoint(const Point* verts, double u, double v, Point& pt, udword* nearvtx=null)	const;
				double			Angle(const IndexedTriangle& tri, const Point* verts)				const;
