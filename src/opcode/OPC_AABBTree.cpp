...
udword AABBTreeNode::Split(udword axis, AABBTreeBuilder* builder)
{
	...
	double SplitValue = builder->GetSplittingValue(mNodePrimitives, mNbPrimitives, mBV, axis);
	...
	for(udword i=0;i<mNbPrimitives;i++)
	{
		...
		double PrimitiveValue = builder->GetSplittingValue(Index, axis);
...
bool AABBTreeNode::Subdivide(AABBTreeBuilder* builder)
{
	...
	else if(builder->mSettings.mRules & SPLIT_SPLATTER_POINTS)
	{
		...
		Means/=double(mNbPrimitives);

		// Compute variances
		Point Vars(0.0, 0.0, 0.0);
		for(udword i=0;i<mNbPrimitives;i++)
		{
			udword Index = mNodePrimitives[i];
			double Cx = builder->GetSplittingValue(Index, 0);
			double Cy = builder->GetSplittingValue(Index, 1);
			double Cz = builder->GetSplittingValue(Index, 2);
			Vars.x += (Cx - Means.x)*(Cx - Means.x);
			Vars.y += (Cy - Means.y)*(Cy - Means.y);
			Vars.z += (Cz - Means.z)*(Cz - Means.z);
		}
		Vars/=double(mNbPrimitives-1);
	...
	else if(builder->mSettings.mRules & SPLIT_BALANCED)
	{
		...
		double Results[3];
		NbPos = Split(0, builder);	Results[0] = double(NbPos)/double(mNbPrimitives);
		NbPos = Split(1, builder);	Results[1] = double(NbPos)/double(mNbPrimitives);
		NbPos = Split(2, builder);	Results[2] = double(NbPos)/double(mNbPrimitives);
		Results[0]-=0.5;	Results[0]*=Results[0];
		Results[1]-=0.5;	Results[1]*=Results[1];
		Results[2]-=0.5;	Results[2]*=Results[2];
		udword Min=0;
